...
def _safe(model):
    """Filesystem-safe single-path-segment token for a model name (HF names have '/')."""
    return model.replace("/", "__").replace(os.sep, "__")

class ActivationStore:
    def __init__(self, store_dir):
        self.dir = store_dir
    def _mdir(self, model):
        return os.path.join(self.dir, _safe(model))
